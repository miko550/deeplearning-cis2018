...
class DataPreprocessor:
    ...
    def __init__(self, csv_path: str = "csv/", test_size: float = 0.2, 
                 ...
                 min_samples_per_class: int = 100, remove_rare_classes: bool = True):
        """
        ...
        Args:
            ...
            min_samples_per_class: Minimum samples required to keep a class (default: 100)
            remove_rare_classes: Whether to remove classes below min_samples_per_class (default: True)
        """
        ...
        self.remove_rare_classes = remove_rare_classes
        self.df = None
    ...
    def handle_rare_classes(self, X: pd.DataFrame, y: pd.Series) -> Tuple[pd.DataFrame, pd.Series]:
        """
        Handle rare classes by removing or analyzing them.
        ...
        """
        if not self.remove_rare_classes:
            return X, y
        
        print(f"\n--- Handling Rare Classes (min_samples={self.min_samples_per_class}) ---")
        ...
        if rare_classes:
            print(f"Found {len(rare_classes)} rare classes below threshold:")
            total_removed = 0
            for class_name in rare_classes:
                count = class_counts[class_name]
                percentage = (count / len(y)) * 100
                print(f"  '{class_name}': {count:,} samples ({percentage:.4f}%)")
                total_removed += count
            ...
            removed_count = original_count - len(y_filtered)
            
            print(f"\n  Removed {removed_count:,} samples ({removed_count/original_count*100:.2f}%)")
            print(f"  Remaining samples: {len(y_filtered):,}")
            
            return X_filtered, y_filtered
        else:
            print("  No rare classes found. All classes have sufficient samples.")
            return X, y
    
    def encode_labels(self) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
        """
        Separate features and labels, handle rare classes, then encode labels.
        ...
        # Extract unique class names before handling rare classes
        unique_classes = sorted(y.unique().tolist())
        print(f"Found {len(unique_classes)} unique classes before filtering:")
        for i, class_name in enumerate(unique_classes):
            class_count = (y == class_name).sum()
            print(f"  {i}: {class_name} ({class_count:,} instances)")
        
        # Handle rare classes (remove if enabled)
        X, y = self.handle_rare_classes(X, y)
        ...
        print(f"\n✓ Labels encoded. Final class mapping:")
        for i, class_name in enumerate(self.class_names):
            class_count = (y_encoded == i).sum()
            print(f"  {i} -> {class_name} ({class_count:,} samples)")
...
def main():
    ...
    RANDOM_STATE = 42
    MIN_SAMPLES_PER_CLASS = 100  # Remove classes with fewer than this many samples
    REMOVE_RARE_CLASSES = True   # Set to False to keep all classes
    ...
    preprocessor = DataPreprocessor(
        ...
        remove_rare_classes=REMOVE_RARE_CLASSES
    )
